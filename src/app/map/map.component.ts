  ...
  addEventsOnMap() {
    ...
    this.map.on('pointermove', (e) => {
      ...
      if (feature) {
        ...
        this.hoveredPlace = this.locationList.find(
          (place: Destination) => place.id == placeId
        );
        button.click();
  ...
  markerLayers(destination: Destination[]) {
    ...
    for (let i = 0; i < destination.length; i++) {
      let iconFeature = new Feature({
        geometry: new Point(
          olProj.transform(
            [destination[i].lon, destination[i].lat],
            'EPSG:4326',
            'EPSG:3857'
          )
        ),
    ...
    let iconStyle = new Style({
      image: new Icon({
        ...
        opacity: 1,
        src:
          'https://icons.iconarchive.com/icons/icons8/windows-8/16/Maps-Geo-Fence-icon.png',
      }),
    });
